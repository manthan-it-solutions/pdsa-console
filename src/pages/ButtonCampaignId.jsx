...
const ButtonIdCompaign  = () => {
  ...
  const getTemplates = async (page = 0, limit = 10) => {
    ...
    try {
      const res = await apiCall({
        endpoint: `groups/get-getButton_compaign-compaignId?page=${page + 1}&limit=${rowsPerPage}`,
        method: 'POST',
        payload: formData
      });
  ...
  const filteredTemplates = isTemplate.filter(
    (template) =>
      template.compaign_id.toString().includes(searchTerm) ||
      (template.bussiness_number &&
        template.bussiness_number.toString().includes(searchTerm))
  );
  ...
  return (
    <>
      <div className="Template_id_contian1">
        <h4 className="Head_titleTemplate"> View Button Campaign
        <input
            placeholder="Search "
            type="text"
...
export default ButtonIdCompaign ;
